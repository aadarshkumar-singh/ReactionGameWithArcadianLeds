...
#include "OS_ReactionGame_ee_internal.inc"

#if (__OO_configUSE_TRACE_FACILITY == 1)
#include "trcKernelPort.h" 
static void EE_oo_creatTaskInfo(void);
#endif
...
static void EE_oo_creatTaskInfo(void)
{
    ...
    {
        traceTASK_CREATE(i, EE_TASK_NAME[i], EE_th_ready_prio[i]);
    }
    for(int i=0;i< EE_MAX_ALARM; i++)
    {
        traceALARM_CREATE(EE_ALARM_ID[i], EE_ALARM_NAME[i], 0x55);
    }
#ifdef EE_MAX_RESOURCE
    for(int i=0;i< EE_MAX_RESOURCE; i++)
    {
        traceRESOURCE_CREATE(EE_RESOURCE_ID[i], EE_RESOURCE_NAME[i], 0x55);
    }
#if (RES1 == 1)
    traceRESOURCE_CREATE(RES_SCHEDULER, "Scheduler", 0x55);
#endif
#endif
}
#endif
...
/* If MemMap.inc support is enabled (i.e. because memory protection): use it */
...
__INLINE__ void   EE_oo_call_StartupHook(void);
__INLINE__ void   EE_oo_call_StartupHook(void) {}
...
__INLINE__ void  EE_oo_start_os(void);
...
StatusType EE_oo_StartOS(AppModeType Mode)
{
  ...
  extern void EE_cortex_mx_svc_ISR(void);
  CyRamVectors[ CY_INT_SVCALL_IRQN ]      = EE_cortex_mx_svc_ISR;       /* SV Call Interrupt           */
  CyRamVectors[ CY_INT_PEND_SV_IRQN ]     = EE_cortex_mx_pendsv_ISR;    /* PenSV Interrupt             */
  #ifdef __USE_SYSTICK__
  CyRamVectors[ CY_INT_SYSTICK_IRQN ]     = EE_CORTEX_MX_SYSTICK_ISR;   /* Systick Interrupt           */
  #endif
  #endif
  }
    ...
    EE_oo_creatTaskInfo();
      ...
      if ( rq != EE_NIL ) {
        ...
        /* Since we are into the StartOS, the task was NOT previously on
          ...
          Look at EE_oo_run_next_task in ee_internal.inc to see the usual
          differences. */
